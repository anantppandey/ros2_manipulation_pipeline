...
*Fixed a startup synchronization issue that caused the robot arm to be incorrectly detected as an obstacle in MoveIt's Octomap. The issue occurred when joint_state_broadcaster failed to activate before the perception pipeline started, preventing /joint_states from being published and causing MoveIt's self-filter to classify the robot's own links as obstacles. Implemented a robust controller initialization sequence that verifies controller states before continuing launch, ensuring reliable robot state publication and consistent Octomap filtering.