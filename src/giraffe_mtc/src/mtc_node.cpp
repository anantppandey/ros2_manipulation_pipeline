                                ...
                                Eigen::Quaterniond& achieved_orientation)
{
    ...
    {
        const double relaxed_max_perturb_deg = 15.0;
...
private:
    ...
    void execute(const std::shared_ptr<GoalHandlePickPlace> goal_handle)
    {
        ...
        double GRIPPER_X_OFFSET = -0.0225;
        double GRIPPER_Y_OFFSET = -0.0225;
        ...
        pipeline->setProperty("max_velocity_scaling_factor", 0.8);
        pipeline->setProperty("max_acceleration_scaling_factor", 0.8);
 
        auto gripper_pipeline = std::make_shared<mtc::solvers::JointInterpolationPlanner>();
        gripper_pipeline->setMaxVelocityScalingFactor(1.0);
        gripper_pipeline->setMaxAccelerationScalingFactor(1.0);
        ...
        auto runSubTrajectory = [&](const mtc::SubTrajectory* traj) {
            ...
            if (group_name == "gripper") {
                ...
                if (stage_name == "lift up") {
                    ...
                    // Only mirror the attach into MoveIt's planning scene once
                    // the cube is confirmed physically attached in Gazebo -
