...
int main(int argc, char *argv[])
{
    ...
    // ==========================================
    // 1. RViz Cube Offsets (Make the green cube match the red point cloud)
    double CUBE_X_OFFSET = -0.005;     
    double CUBE_Y_OFFSET = 0.0;     
    double CUBE_Z_OFFSET = -0.07;     

    // 2. Gazebo Gripper Target Offsets (Make the gripper go to the right place)
    double GRIPPER_X_OFFSET = -0.031; 
    double GRIPPER_Y_OFFSET = -0.0275;
    double GRIPPER_Z_OFFSET = 0.015;  

    // 3. Motion distances
    double DESCEND_DISTANCE = 0.05; // How far down to descend to the cube
    double LIFT_DISTANCE = 0.10;    // How high to lift after attaching
    ...
    // ==========================================
    // COMPUTE GRASP POSE IK (DESCEND_DISTANCE lower)
    // ==========================================
    Eigen::Vector3d grasp_pos = target_pos - Eigen::Vector3d(0, 0, DESCEND_DISTANCE);
    ...
    // ==========================================
    // COMPUTE LIFT POSE IK (LIFT_DISTANCE higher than grasp)
    // ==========================================
    Eigen::Vector3d lift_pos = grasp_pos + Eigen::Vector3d(0, 0, LIFT_DISTANCE);
    ...
    // ADD CUBE TO MTC's INTERNAL SCENE so the Attach stage works!
    ...
    close_gripper->setGroup("gripper");
    std::map<std::string, double> gripper_close;
    gripper_close["wrist_2_gripper_joint"] = 0.45; 
    close_gripper->setGoal(gripper_close);
    close_gripper->setProperty("timeout", 5.0);
    task.add(std::move(close_gripper));
    ...
    // ==========================================
    auto attach_cube = std::make_unique<mtc::stages::ModifyPlanningScene>("attach cube");
    // Just pass the object name and the link name
            ...
            for (const auto& sub : compound->solutions()) {
                ...
                if (traj && traj->trajectory()) {
                    ...
                    if (group_name == "gripper") {
                        ...
                        // MANUALLY ATTACH CUBE BEFORE LIFTING!
                        // We must use arm_group because it has the end_effector context.
